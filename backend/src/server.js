...
app.put("/api/products/:id", asyncRoute(async (req, res) => {
  ...
  try {
    ...
    await client.query(
      "update detalle_compra set precio_venta = $1 where id_producto = $2",
      [price, req.params.id]
    );

    await client.query(
      "update producto_proveedor set precio_compra = round(($1::numeric * 0.78), 2) where id_producto = $2",
      [price, req.params.id]
    );
...
app.get("/api/reports", asyncRoute(async (_req, res) => {
  ...
  res.json({
    cards: [
      ...
      { title: "Proveedores vinculados", value: supplierCount[0]?.total || 0, detail: "Relaciones activas con productos actuales" },
      { title: "Inventario observado", value: "En tiempo real", detail: "Compras y ediciones actualizan la informacion mostrada" },
    ],
    topProducts,
    topClients,
    sections: [
      {
        ...
        rows: joinProductSales.map((row) => [row.producto, row.categoria, row.proveedor, row.unidades, row.total]),
      },
      {
      ...
      },
      {
        ...
        rows: joinSupplierInventory.map((row) => [row.proveedor, row.producto, row.categoria, row.stock, row.costo]),
      },
      {
        ...
        rows: productsBelowAverageStock.map((row) => [row.producto, row.categoria, row.stock, row.promedio]),
      },
      {
        ...
        rows: productsWithSalesExists.map((row) => [row.producto, row.categoria, row.precio, row.stock]),
      },
      {
      ...
      },
      {
        ...
        rows: topProducts.map((row) => [row.producto, row.categoria, row.unidades, row.total, row.supplier]),
      },
    ],
  });
}));
